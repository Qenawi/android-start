...
public class MyUploadService extends MyBaseTaskService {
    ...
    private void uploadFromUri(final Uri fileUri) {
        ...
        final StorageReference photoRef = mStorageRef.child("photos").child(mFirebaseUser.getEmail()+"_p_photo");

        // [END get_child_ref]
        // Upload file to Firebase Storage
        Log.d(TAG, "uploadFromUri:dst:" + photoRef.getPath());
   //     Log.d(TAG, "getLast pathSegment->" + fileUri.getLastPathSegment());
        // photo ref.put fle -> upload finished
        photoRef.putFile(fileUri)
                .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>()
                {
                    @Override
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                        // DAtasnapshot  snapshot of the upload and the file is
                        // uploaded to the server. This snapshot gives you useful information
                        // Upload succeeded
    ...
    private void showUploadFinishedNotification(@Nullable Uri downloadUrl, @Nullable Uri fileUri) {
        ...
        int icon = success ? R.drawable.ic_account_circle_black_36dp : R.drawable.common_google_signin_btn_icon_light;
